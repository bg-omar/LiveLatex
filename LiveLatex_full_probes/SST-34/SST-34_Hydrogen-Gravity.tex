...
\label{sec:legacy-rosetta}
    ...
    \subsection{Quantized Circulation and Mass Formula}
        ...
        \begin{equation}
            \oint \vec{v} \cdot d\vec{\ell} = n \, \kappa_{\ae}
        \end{equation}
        with velocity expressed as a gradient,
        \begin{equation}
            \vec{v} = \lambda_{\ae} \, \nabla \theta, \qquad \psi = \sqrt{\rho/\rho_{\ae}}\, e^{i\theta},
        \end{equation}
        leading to a hydrodynamic Schrödinger equation with a swirl potential,
        \begin{equation}
            i\hbar_{\ae} \, \frac{\partial \psi}{\partial t} = -\frac{\hbar_{\ae}^2}{2m_{\ae}} \, \nabla^2 \psi + \Phi_{\text{swirl}}(\vec{\omega})\, \psi,
        \end{equation}
        \begin{equation}
            \Phi_{\text{swirl}} = \frac{1}{2} \, \lambda_g \, \rho_{\ae} \, |\vec{\omega}|^2.
        \end{equation}
...
\end{table}
                ...
                \begin{tikzpicture}[use Hobby shortcut, rotate=4]
                \tikzset{knot diagram/every strand/.append style={draw=blue}}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\linewidth]{figures/KT.png}
  \caption{KT}
  \label{fig:kt}
\end{figure}
