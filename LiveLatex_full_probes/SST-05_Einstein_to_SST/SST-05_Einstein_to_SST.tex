...
\providecommand{\institutionalreview}[1]{}
\providecommand{\informedconsent}[1]{}
\providecommand{\dataavailability}[1]{}
\providecommand{\supplementary}[1]{}
\providecommand{\conflictsofinterest}[1]{}
\providecommand{\abbreviations}[2]{}
\providecommand{\reftitle}[1]{}
...
\begin{figure}[htbp]
    ...
    \resizebox{\textwidth}{!}{%
        ...
        \begin{tikzpicture}
            ...
            \draw[->, thick] (7.5,0) -- (7.5,0.9);   % (slot used for Leibniz/Mach panel)
            \draw[->, thick] (11.4,0) -- (11.4,1.9); % Swirl–String

            % Arrows below timeline (short)
            \draw[->, thick] (1.0,0) -- (1.0,-0.9);     % Plato/Aristotle
            \draw[->, thick] (5.5,0) -- (5.5,-1.8);     % Newton
            \draw[->, thick] (10.4,0) -- (10.4,-0.9);   % Einstein (SR/GR window)

            %--- Root date cards (above timeline) ---
            ...
            %--- Root date cards (below timeline) ---
            ...
            \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=6.2cm] at (11.4,3.0) {
                \textbf{Swirl--String: Layered Time}\\
                Absolute foliation: \(\Nabs\).\\
                Swirl clock: \(\Sswirl(\Nabs)=\displaystyle\int_{0}^{\Nabs}\!\Omega\,d\Nabs\).\\
                Local rate: \(\dfrac{d\ptau}{d\Nabs}=\StOf{\vt}=\sqrt{1-\vt^{2}/c^{2}}\).\\
                Topological updates: \(\Ktop\) at reconnection.
            };

            % --- Plato / Aristotle ---
            \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=6.3cm] at (0.8,-1.5) {
                \textbf{Plato / Aristotle: Time as Motion/Image}\\
                Plato: time mirrors eternity.\\
                Aristotle: time \(=\) change.
            };

            % --- Einstein ---
            \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=5.7cm] at (10.9,-1.8) {
                \textbf{Einstein: Relativized Time}\\
                Time depends on motion and potential.\\
                Simultaneity via light signals;\\
                proper time \(\tau\) is path--dependent.\\
                \(t_B=\tfrac{1}{2}\big(t_{\text{send}}+t_{\text{return}}\big)\).
            };

            % --- Newton ---
            \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=6.2cm] at (5.5,-2.5) {
                \textbf{Newton: Absolute Time \& Simultaneity}\\
                Time flows uniformly and independently\\
                of the observer; simultaneity is universal.
            };

        \end{tikzpicture}
    }
        \caption{\textbf{Chronology of simultaneity theories across physics and philosophy.} From ancient views (change vs.\ inner sense), through Newton’s absolute simultaneity and Einstein’s frame--dependent proper time, to a swirl--string causal layering. The model introduces a physically grounded sequence culminating in measurable, observer--dependent time \((\tau)\) and discrete topological updates \((\Ktop)\).}\label{fig:history-time-simultaneity}
\end{figure}
        ...
        \resizebox{\textwidth}{!}{%
            ...
            \begin{tikzpicture}
                ...
                \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=5.4cm] at (7.5,1.5) {
                    \textbf{Einstein: Light Quanta}\\
                    Photoelectric effect implies photons.\\
                    Energy quantized: \(E=hf\).
                };

                % --- Swirl–String (modern) ---
                \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=6.2cm] at (11.4,2.8) {
                    \textbf{Swirl--String: Unified Filamentary Excitations}\\
                    Excitations = quantized filamentary circulation\\
                    in structured space. Wave--like behavior from\\
                    phase interference; particle--like from localized\\
                    topology (knots/links). Single substrate; no dualism.
                };

                % --- Aristotle ---
                \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=5.4cm] at (1.4,-1.5) {
                    \textbf{Aristotle: Continuous Medium}\\
                    Matter is continuous;\\
                    motion propagates as waves.
                };

                % --- Young / Huygens ---
                \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=5.4cm] at (5.5,-2.8) {
                    \textbf{Young / Huygens: Wave Optics}\\
                    Light as waves in a structured space.\\
                    Interference and diffraction.
                };

                % --- de Broglie / Schrödinger ---
                \node[draw, rounded corners=2pt, thick, align=center, fill=white, text width=5.7cm] at (10.2,-1.5) {
                    \textbf{de Broglie / Schr\"odinger: Duality}\\
                    Matter has wavelength \(\lambda=h/p\).\\
                    Wavefunction governs evolution.
                };

            \end{tikzpicture}
        }
            \caption{\texorpdfstring{\textbf{Intellectual trajectory of wave--particle duality:} from corpuscles and classical waves to quantum dualities and a unified filamentary picture. In a swirl--string framework, excitations are quantized circulation in a structured, incompressible, inviscid medium; wave and particle aspects are limits of the same dynamical structure.}}\label{fig:WaveParticleDuality}
        \end{figure}
...
%—— Additional Literature (fluid, analog, superfluid) ——
            ...
            %--- Add missing bibliography item inside the existing thebibliography block ---
